...
## Экранная тема и граница печатного слоя

Основной `frontend/styles/app.css` остаётся владельцем функциональной компоновки, адаптивного представления и печатных шаблонов. Визуальный язык Frontend Reference Kit v1 подключён последующим файлом `frontend/styles/reference-theme.css`. Все правила этого файла находятся внутри `@media screen`, используют только локальные ресурсы из `frontend/assets` и не участвуют в печати.

Такое разделение сохраняет принятые печатные контракты: Segoe UI/Arial, 14 pt для производственного отчёта и структуры, A4/A3 landscape. Профиль и отличия переноса зафиксированы в `frontend-reference-profile.json` и `frontend-reference-deviations.json`.
...
- Отдельная вкладка строит диаграмму Ганта по датам этапов.
- Масштаб Ганта меняет только временную сетку: календарные дни, недели с понедельника, календарные месяцы или календарные кварталы. Поквартальный масштаб выбран по умолчанию; даты этапов не пересчитываются.
- Колонка названий закреплена и изолирована от временного поля. Длинные названия сокращаются многоточием, полный текст доступен через стандартную подсказку.
