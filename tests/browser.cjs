...
(async()=>{
 ...
 try{
  ...
  await firstStage.locator('[data-act="edit"]').click();assert.equal(await page.inputValue('#stageAddressees'),'Адресат этапа А');assert.equal(await page.inputValue('#stageComment'),'Сохранённый комментарий этапа');await page.locator('#stageModal [data-close="stageModal"]').click();
  await firstStage.locator('[data-act="add"]').click();assert.equal(await page.inputValue('#stageExecutor'),'Исполнитель А');assert.equal(await page.inputValue('#stageAddressees'),'Адресат этапа А');assert.equal(await page.inputValue('#stageStart'),'2026-06-01');assert.equal(await page.inputValue('#stageDeadline'),'2026-06-30');assert.equal(await page.inputValue('#stageComment'),'');await page.locator('#stageModal [data-close="stageModal"]').click();
