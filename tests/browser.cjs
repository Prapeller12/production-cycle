...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{window.__printed=false;window.print=()=>{window.__printed=document.body.classList.contains('print-gantt')&&document.getElementById('ganttPageStyle').textContent.includes('A3 landscape')}});await page.click('#printGanttA3Btn');await page.waitForFunction(()=>window.__printed);assert.match(await page.locator('#ganttPrint').innerText(),/Формат A3/);await page.evaluate(()=>window.dispatchEvent(new Event('afterprint')));
