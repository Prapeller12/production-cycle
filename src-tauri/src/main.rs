...
fn main(){
    ...
    tauri::Builder::default()
        ...
        .register_uri_scheme_protocol("production",move |_context,request|{
            ...
            Response::builder().status(if body.is_some(){200}else{404}).header("Content-Type",mime).header("Cache-Control","no-store").body(body.unwrap_or_default()).unwrap()
        })
        .invoke_handler(tauri::generate_handler![commands::save_file,commands::print_report])
        .setup(|app|{
