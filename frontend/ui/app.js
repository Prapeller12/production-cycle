...
let editStageUid=null, previewKind='out', exportCache=null,completionFilter='active',activeView='registry',ganttScale='week',structureCollapsed=false,importFiles={out:null,in:null},importCandidate=null,importPreviewKind='out',dictionaryEntries=[],projectListItems=[];
...
async function saveProject(){try{const snapshot=J.fingerprint(state);if(B.native()){const v=await B.validate(state);if(v.errors.length){await showValidation();return}const isNew=!Number.isSafeInteger(state.databaseId);state.databaseId=await B.save(state);savedSnapshot=snapshot;updateSavedState();await refreshAutocomplete();message('ok',isNew?'Проект сохранён как отдельная запись локальной базы данных.':'Изменения проекта сохранены в его записи базы данных.');return}const payload=J.stringify(state);const confirmed=await downloadText(`Производственный цикл ${fileSafe(state.project.orderNo)}.json`,payload,'application/json;charset=utf-8');if(confirmed){savedSnapshot=snapshot;updateSavedState()}else message('ok','JSON передан браузеру для сохранения. Проверьте папку загрузок.');await refreshAutocomplete()}catch(e){message('err','Не удалось сохранить: '+e)}}
...
function newProject(){if((state.project.name||state.stages.length)&&!confirm('Очистить текущий проект и начать новый?'))return;state={databaseId:null,project:{name:'',orderNo:'',initiator:'',executor:'',addressees:'',start:'',deadline:''},stages:[],nextSeq:1,collapsed:{}};syncForm();render();refreshAutocomplete();message('ok','Создан новый пустой проект.')}
