...
let editStageUid=null, previewKind='out', exportCache=null,completionFilter='active',activeView='registry',ganttScale='week',structureCollapsed=false,importFiles={out:null,in:null},importCandidate=null,importPreviewKind='out',dictionaryEntries=[],projectListItems=[],auditUsers=[],pendingSignatureAction=null,persistedStatuses=new Map();
...
function newlyCompletedStages(){return state.stages.filter(stage=>stage.status==='done'&&persistedStatuses.get(stage.uid)!=='done')}
...
function makeStageRow(s,depth){const tr=document.createElement('tr');const st=derivedStageVisualStatus(s),dl=daysLeft(s.deadline),kids=hasChildren(s.uid),deadlineState=deadlineRelation(s);if(deadlineState==='beyond')tr.classList.add('deadline-beyond-project');const deadlineWarning=deadlineState==='equal'?'<span class="deadline-warning" title="Дедлайн этапа равен дедлайну основного проекта" aria-label="Дедлайн этапа равен дедлайну основного проекта">!</span>':'';tr.innerHTML=`
...
<td>${escapeHtml(s.executor)}</td><td>${escapeHtml(s.addressees||'—')}</td><td><span class="status-badge ${st}"><span class="status-dot ${st}"></span>${statusText(st)}</span></td><td>${fullDate(s.start)}</td><td class="deadline ${dl!==null&&dl<0&&st!=='done'?'overdue':''}">${fullDate(s.deadline)}</td><td class="left ${dl!==null&&dl<0&&st!=='done'?'overdue':dl!==null&&dl<=7?'soon':''}">${st==='done'?'—':dl}${deadlineWarning}</td>`;
...
function openStageModal(parentUid=null,editUid=null){editStageUid=editUid;const s=editUid?byUid(editUid):null,source=s||(parentUid?byUid(parentUid):state.project)||state.project;$('stageModalTitle').textContent=s?'Редактирование этапа':'Новый этап';
 ...
 $('stageTitle').value=s?s.title:'';$('stageExecutor').value=source.executor||'';$('stageAddressees').value=source.addressees||'';$('stageStart').value=source.start||isoToday();$('stageDeadline').value=source.deadline||'';$('stageStatus').value=s?s.status:'new';$('stageComment').value=s?(s.comment||''):'';parentSel.value=s?(s.parentUid||''):(parentUid||'');
...
function saveStage(){const title=clean($('stageTitle').value),executor=clean($('stageExecutor').value),addressees=clean($('stageAddressees').value),start=$('stageStart').value,deadline=$('stageDeadline').value,status=$('stageStatus').value,comment=clean($('stageComment').value),parentUid=$('stageParent').value||null;if(!title||!executor||!addressees||!start||!deadline||!status){message('err','Заполните все обязательные поля этапа.');return}if(start>deadline){message('err','У этапа дата начала не может быть позже дедлайна.');return}
 ...
 const completing=status==='done'&&(!editStageUid||persistedStatuses.get(editStageUid)!=='done');if(editStageUid){const s=byUid(editStageUid);Object.assign(s,{title,executor,addressees,start,deadline,status,comment,parentUid})}else{const siblings=childrenOf(parentUid);state.stages.push({uid:uid(),seq:state.nextSeq++,sort:siblings.length+1,parentUid,title,executor,addressees,start,deadline,status,comment})}closeModal('stageModal');render();refreshAutocomplete();message('ok',completing?'Этап изменён. Для фиксации статуса «Выполнено» подпишите сохранение.':'Этап сохранён.');if(completing&&B.native())saveProject()}
...
function userOptions(users,adminsOnly=false){const values=adminsOnly?users.filter(user=>user.role==='admin'&&user.active):users.filter(user=>user.active);return values.map(user=>`<option value="${user.id}">${escapeHtml(user.displayName)} · ${escapeHtml(user.keyFingerprint)}${user.role==='admin'?' · администратор':''}</option>`).join('')}
async function refreshAuditUsers(){auditUsers=await B.listAuditUsers();$('saveSigner').innerHTML=userOptions(auditUsers);$('authorizingAdmin').innerHTML='<option value="">—</option>'+userOptions(auditUsers,true);$('verifyAdmin').innerHTML=userOptions(auditUsers,true);$('auditUsers').innerHTML=auditUsers.length?auditUsers.map(user=>`<div class="audit-user"><div><strong>${escapeHtml(user.displayName)}</strong><span>${user.role==='admin'?'Администратор':'Подтверждающий'}</span></div><code>${escapeHtml(user.keyFingerprint)}</code></div>`).join(''):'<div class="projects-empty">Профилей пока нет. Первый профиль автоматически станет администратором.</div>';return auditUsers}
async function openSignature(action){
 if(!B.native()){await action.browserFallback?.();return}
 try{await refreshAuditUsers();if(!auditUsers.length){message('err','Сначала создайте профиль администратора и ключ подписи.');await openAudit();$('createUserDetails').open=true;return}pendingSignatureAction=action;$('saveSignerPin').value='';$('saveChangeComment').value=action.defaultComment||'';$('evidenceType').value='';$('evidenceReference').value='';$('evidenceComment').value='';$('saveConfirmationError').innerHTML='';const needsEvidence=action.type==='save'&&newlyCompletedStages().length>0;$('completionEvidence').classList.toggle('visible',needsEvidence);openModal('saveConfirmationModal')
 }catch(e){message('err','Не удалось подготовить подпись: '+e)}
}
async function confirmSignedAction(){
 const action=pendingSignatureAction;if(!action)return;const userId=Number($('saveSigner').value),pin=$('saveSignerPin').value,comment=clean($('saveChangeComment').value),needsEvidence=action.type==='save'&&newlyCompletedStages().length>0;
 if(!userId||!pin||!comment){$('saveConfirmationError').innerHTML='<div class="notice err">Выберите подтверждающего, введите PIN и обязательный комментарий.</div>';return}
 let evidence=null;if(needsEvidence){evidence={documentType:$('evidenceType').value,documentReference:clean($('evidenceReference').value),comment:clean($('evidenceComment').value)};if(!evidence.documentType||!evidence.documentReference||!evidence.comment){$('saveConfirmationError').innerHTML='<div class="notice err">Для выполненного этапа заполните тип, реквизиты официального документа и комментарий.</div>';return}}
 $('confirmSignedSaveBtn').disabled=true;
 try{
  if(action.type==='save'){const isNew=!Number.isSafeInteger(state.databaseId),result=await B.saveSigned(state,{userId,pin,comment,evidence});state.databaseId=result.projectId;savedSnapshot=J.fingerprint(state);rememberPersistedStatuses();updateSavedState();await refreshAutocomplete();message('ok',isNew?'Проект подписан и сохранён как отдельная запись.':'Изменения подписаны и сохранены в журнале.')}
...
function auditEventLabel(type){return ({project_created:'Создан проект',project_changed:'Изменён проект',stage_completed:'Подтверждено выполнение',dictionary_replaced:'Изменён справочник'})[type]||type}
async function loadAuditEvents(){try{const events=await B.listAuditEvents(Number.isSafeInteger(state.databaseId)?state.databaseId:null);$('auditEvents').innerHTML=events.length?events.map(event=>`<article class="audit-event"><div><b>#${event.seq} · ${escapeHtml(auditEventLabel(event.eventType))}</b><time>${escapeHtml(confirmationDate(event.createdAt))}</time></div><p>${escapeHtml(event.comment)}</p><small>${event.stageRegNumber?`Этап ${escapeHtml(event.stageRegNumber)} · `:''}${event.evidenceType?`${escapeHtml(event.evidenceType)}: ${escapeHtml(event.evidenceReference||'')} · `:''}${escapeHtml(event.actorName)} · ключ ${escapeHtml(event.keyFingerprint)}</small></article>`).join(''):'<div class="projects-empty">Для выбранного проекта событий пока нет.</div>'}catch(e){$('auditEvents').innerHTML=`<div class="notice err">${escapeHtml(e)}</div>`}}
async function openAudit(){if(!B.native()){message('err','Журнал подписей доступен в Windows-приложении.');return}openModal('auditModal');$('auditVerification').innerHTML='';try{await refreshAuditUsers();await loadAuditEvents()}catch(e){$('auditUserError').innerHTML=`<div class="notice err">${escapeHtml(e)}</div>`}}
async function createAuditUser(){const input={displayName:clean($('newAuditUserName').value),pin:$('newAuditUserPin').value,isAdmin:$('newAuditUserRole').value==='admin',adminUserId:$('authorizingAdmin').value?Number($('authorizingAdmin').value):null,adminPin:$('authorizingAdminPin').value||null};try{const user=await B.createAuditUser(input);$('newAuditUserName').value='';$('newAuditUserPin').value='';$('authorizingAdminPin').value='';$('auditUserError').innerHTML=`<div class="notice ok">Создан профиль «${escapeHtml(user.displayName)}», ключ ${escapeHtml(user.keyFingerprint)}.</div>`;await refreshAuditUsers()}catch(e){$('auditUserError').innerHTML=`<div class="notice err">${escapeHtml(e)}</div>`}}
...
async function loadProjectById(projectId){try{const candidate=await B.loadById(projectId);if(!candidate){message('err','Проект не найден.');return}if(J.fingerprint(state)!==savedSnapshot&&(state.project.name||state.stages.length)&&!confirm('Заменить текущий проект данными из локальной базы?'))return;state=candidate;savedSnapshot=J.fingerprint(state);rememberPersistedStatuses();syncForm();closeModal('projectsModal');render();await refreshAutocomplete();message('ok',`Открыт проект «${state.project.name}», заказ № ${state.project.orderNo}.`)}catch(e){message('err','Не удалось открыть проект: '+e)}}
...
$('saveStageBtn').onclick=saveStage;$('pdfReportBtn').onclick=printPdfReport;$('validateBtn').onclick=showValidation;$('exportBtn').onclick=openExport;$('saveJsonBtn').onclick=saveProject;$('loadJsonBtn').onclick=showProjects;$('dictionaryBtn').onclick=openDictionary;$('auditBtn').onclick=openAudit;$('confirmSignedSaveBtn').onclick=confirmSignedAction;$('createAuditUserBtn').onclick=createAuditUser;$('refreshAuditBtn').onclick=loadAuditEvents;$('verifyAuditBtn').onclick=verifyAudit;$('importBtn').onclick=openImport;$('jsonFile').onchange=e=>{if(e.target.files[0])loadJsonFile(e.target.files[0]);e.target.value=''};$('newBtn').onclick=newProject;$('demoBtn').onclick=loadDemo;
$('dictionaryKind').onchange=loadDictionary;$('dictionarySearch').oninput=renderDictionary;
$('projectsSearch').oninput=renderProjectList;
